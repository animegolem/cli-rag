...
The .toml is designed to be very extensible but we will create a [[ADR-010-LUA-advanced-config]] escape hatch for highly custom use-cases. 
...
#:  +-----------------------+--------------------------------------------------+
[schema.new]
id_generator = { strategy = "increment", prefix = "ADR-" padding = "3" }#: Define the name template for the `new` command. unset = filename
