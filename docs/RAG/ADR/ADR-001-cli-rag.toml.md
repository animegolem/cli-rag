...
default_fanout = 5
#: Maximum total nodes in returned output. default 50
default_max_nodes = 50
#: Whether to include dependents (backlinks) in traversals. default = true
include-bidirectional = true
...
[config.templates]
import = [".cli-rag/templates/ADR.toml", ".cli-rag/templates/ARD-AI.toml",".cli-rag/templates/LOG-AI.toml", ".cli-rag/templates/IMP-AI.toml" ]
```
...
#: Graph edge fields - automatically added to frontmatter. `validate` will
...
#:                      # --- `~.frontmatter.GTD` --- #
#: +-----------------+---------------------------------------------------------+
#: | `kanban_status` | Define Kanban status. Dispalyed on the agenda           |
...
### In-Scope 
- we should aim to have all handles in the toml reflected in a lua accessible api. 
...
On load, build a resolved config object with:
- Snake_case in and snake_case out for Lua
- Provide a JSON-serializable view with camelCase for CLIs that surface config

**Example resolved Lua table**:
```lua
cfg = {
  scan = { filepaths = {...}, index_path = ".cli-rag/index.json", hash_mode = "mtime", index_strategy = "content", ignore_globs = {...}, ignore_symlinks = false },
  authoring = { editor = "neovim", background_watch = true },
  graph = {
   depth = 1, include_bidirectional = true,
   ai = { depth = 1, default_fanout = 5, default_max_nodes = 50, include_bidirectional = true, neighbor_style = "metadata", outline_lines = 2 }
  },
  templates = { import = { ".cli-rag/templates/ADR.toml", ... } },
  schemas = { ... } -- loaded/merged schema blocks
}
```

Engine responsibilities

- Merge policy: Lua overlay deep-merges into TOML; hooks only add behavior.
...
- --no-lua flag and an environment guard (e.g., CLI_RAG_NO_LUA=1) for CI.
- Stable ctx for hooks: { schema, config, index, clock, fs, util }.
- info --json shows: { config: { path, version }, lua: { repoLua: exists, userLua: exists, apiVersion }, capabilities: { aiGet: { retrievalVersion: 1 } } }.
- init --lua scaffold for .cli-rag.lua.
