...
pub fn run(cfg: &Config, format: &str, topic: String, include_content: Option<bool>) -> Result<()> {
    ...
    if format == "json" {
        let members: Vec<GroupMember> = matches
            ...
            .collect();
        let mut out = serde_json::json!({"topic": topic, "count": members.len(), "adrs": members});
        if include_content {
        ...
        let members: Vec<GroupMember> = matches
            .iter()
            .filter_map(|d| d.id.as_ref().map(|id| (id, d)))
            .map(|(id, d)| GroupMember { id: id.clone(), title: d.title.clone(), status: d.status.clone(), groups: d.groups.clone(), file: Some(d.file.clone()) })
            .collect();
